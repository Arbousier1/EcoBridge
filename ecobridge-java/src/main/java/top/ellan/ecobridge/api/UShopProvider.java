...
/**
* UltimateShop 数据适配器 (v0.8.7-Precision-Patch)
* 职责：作为 Java 25 与 UltimateShop 之间的胶水层。
* <p>
* 架构变更日志：
* 1. **Nano-Cache**: [v0.8.7 Fix] 切换至 System.nanoTime() 并将 TTL 提升至 500ms，大幅削减 FFI 调用背压。
* 2. **Per-Shop-Product Cache**: 引入复合键 (Shop:Product)，解决多商店冲突。
* 3. **SSoT Alignment**: 严格遵循 Rust v0.8 models.rs 的内存布局。
*/
public final class UShopProvider {

  // --- Tick-Loop Cache (纳秒级原子快照缓存) ---
  ...
  // 将缓存时长定为 500ms，对应约 10 个 Minecraft Ticks
