...
/**
 * 玩家数据热点缓存 (HotDataCache v0.8.8 - SSoT Read-Only Mirror)
 * 职责：维护在线玩家的高频交易数据快照，作为物理引擎的高性能只读视图。
 * <p>
 * 修复日志 (v0.8.8):
 * - [Safety] 彻底移除 LATE_LOAD_PROTECT 回写逻辑，防止过时快照覆盖数据库最新余额。
...
public class HotDataCache {
            ...
            try {
                ...
                Bukkit.getScheduler().runTask(EcoBridge.getInstance(), () -> {
                    ...
                    if (p != null && p.isOnline()) {
                        ...
                        // [v0.8.8 Fix] 拦截到过时加载回调，不再回写，防止旧快照覆盖数据库中的最新真相
                        LogUtil.debug("拦截到过时加载回调 (" + uuid + ")，玩家已离线，取消回写。");
                    }
                });
            } catch (Exception e) {
    ...
    /**
     * 线程安全的玩家数据容器
     * 使用 AtomicLong 存储 double 的位表示，实现无锁并发读取快照
     */
