...
/**
 * 深度指令接管器 (V0.8.5 兼容性增强版)
 * 职责：拦截并接管所有经济相关的转账指令，转发至 EcoBridge 审计系统。
 * 修复：
 * 1. [兼容性]: 移除了对 "give" 子指令的拦截，防止管理员管理命令失效。
 * 2. [风控]: 增加管理员权限绕过逻辑 (ecobridge.admin.bypass)。
 * 3. [对齐]: 严格遵循 CoinsEngine v0.8.x 的主货币判定逻辑。
 */
public class CommandInterceptor implements Listener {
    ...
    public void onCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        // [v0.8.5 Fix]: 管理员权限绕过，确保后台管理命令不被审计系统误伤
        ...
        if (currency == null) {
            if (label.equals("pay") || label.equals("transfer") || label.equals("epay")) {
                ...
                targetIdx = 1;
                amountIdx = 2;
                // 使用 Stream 查找 isPrimary() 为 true 的货币
        ...
        // 场景 B: 特定货币指令 (如 /coins pay)
        else {
            if (parts.length > 1) {
                String subCmd = parts[1].toLowerCase();
                // [v0.8.5 Fix]: 移除 "give" 和 "set"，仅拦截明确的转账动作 "pay" 和 "send"
