...
class LifecycleComponentTest {
  ...
  void testStartupExcludesReloadAndShutdown() {
    ...
    assertEquals(
        4, startup.size(), () -> "startup should have 4 components (no RELOAD, no SHUTDOWN)");
  ...
  void testEachComponentHasUniquePhase() {
    ...
    List<LifecycleComponent> all = LifecycleCatalog.all();
    assertTrue(
        all.stream().map(LifecycleComponent::phase).distinct().count() >= 4,
  ...
  void testPlatformIntegrationLifecyclePhase() {
    assertEquals(
        LifecyclePhase.PLATFORM_INTEGRATION, PlatformIntegrationLifecycle.INSTANCE.phase());
