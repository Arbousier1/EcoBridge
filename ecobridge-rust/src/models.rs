//! EcoBridge Economy System - Integrated Data Models (SSoT v0.7.4)
//! 
//! # 核心设计准则 (Single Source of Truth)
//! 1. **C-ABI 兼容**: 强制使用 `#[repr(C)]`，禁止编译器重排字段。
//! 2. **扁平化布局**: 修复了 `MarketConfig` 中数组与 Java 独立字段不匹配的隐患。
//! 3. **显式对齐**: 确保所有结构体的大小均为 8 字节的倍数。
...
pub struct PidState {
    ...
    pub lambda: c_double,            // Offset 24 (漏损系数)
    ...
    pub is_saturated: c_int,         // Offset 64 (4 bytes)
    /// [显式填充]: 确保结构体总大小为 72 字节 (8的倍数)
    pub _padding: c_int,             // Offset 68 (4 bytes)
}
...
/// 单条历史交易快照 (16 bytes)
/// 对应 Java 侧的 SaleRecord，用于兼容旧版算法。
#[repr(C)]
